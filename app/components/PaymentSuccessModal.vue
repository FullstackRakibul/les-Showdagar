<template>
  <teleport to="body">
    <transition name="modal">
      <div v-if="checkoutStore.showSuccessModal" class="fixed inset-0 z-[9999] overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 py-4">
          <div class="fixed inset-0 bg-black bg-opacity-50"></div>

          <div class="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-md w-full p-8 text-center">
            <!-- Success Animation -->
            <div class="mb-6">
              <div
            ...
            <!-- Order Details -->
            <div v-if="checkoutStore.currentOrder" class="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 mb-6 text-left">
              <div class="flex justify-between items-center mb-2">
                <span class="text-sm text-gray-600 dark:text-gray-400">Order ID:</span>
                <span class="font-mono text-sm font-medium">{{ checkoutStore.currentOrder.id }}</span>
              </div>
              <div class="flex justify-between items-center mb-2">
                <span class="text-sm text-gray-600 dark:text-gray-400">Total Amount:</span>
                <span class="font-semibold text-lg text-primary">${{ checkoutStore.currentOrder.total.toFixed(2)
                  }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-sm text-gray-600 dark:text-gray-400">Items:</span>
                <span class="text-sm font-medium">{{ checkoutStore.currentOrder.items.length }} item(s)</span>
              </div>
            </div>
            ...
            <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/30 rounded-xl">
              <div class="flex items-center justify-center mb-2">
                ...
                <span class="text-sm font-medium text-blue-800 dark:text-blue-300">Confirmation Email Sent</span>
              </div>
            ...
            <div class="space-y-3">
              ...
              <button @click="checkoutStore.closeSuccessModal"
